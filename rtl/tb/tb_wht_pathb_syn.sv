// tb_wht_pathb_syn.sv — Path B end-to-end, SYNTHESIZABLE tier:
// cq_value_path_wht_syn (rotated) -> wht_inverse_out_syn == reference Vhat.
// Clone of tb_wht_pathb.sv driving the _syn DUTs; must stay bit-exact to the golden vectors
// (and thus to the behavioral oracle).
`timescale 1ns/1ps
`ifndef QD
 `define QD 128
`endif
module tb_wht_pathb_syn;
    localparam int D = `QD, DW = 16;
    reg  [D*DW-1:0] iv;
    wire [D*8-1:0]  codes; wire [DW-1:0] scale;
    reg  [$clog2(D)-1:0] didx;
    wire [DW-1:0]   drot;
    cq_value_path_wht_syn #(.D(D), .DW(DW)) kve (
    ...
    wire [D*32-1:0] vhat;
    wht_inverse_out_syn #(.D(D), .DW(DW)) mate (.rot_out(rotvec), .vhat_out(vhat));
    ...
    initial begin
        ...
        $display("Path B SYN (KVE rotated -> MatE inverse) vs reference Vhat: %0d/%0d bit-exact (D=%0d)", pass, checks, D);
